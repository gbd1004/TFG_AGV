...
with InfluxDBClient(url="http://localhost:8086", token=token, org=org) as client:
    ...
    for salida in [50]:
        model = TCNModel(input_chunk_length=100, output_chunk_length=50)
        # model = TCNModel(
        ...
        train_ed.plot()
        val_ed.plot()
        m_mae, m_mase, m_dtw, t_entrenamiento, t_prediccion = test_univariate_ml_scaled(model=model, nombre="tcn_univ", val_size=VAL_SIZE, salida=salida, series=series_ed_scaled, train_series=train_ed_scaled, val_series=val_ed, val_series_scaled=val_ed_scaled, scaler=scaler_ed)
        print(m_mae)
        print(m_mase)
        print(m_dtw)
        ...
        train_ed.plot()
        val_ed.plot()
        m_mae, m_mase, m_dtw, t_entrenamiento, t_prediccion = test_covariante_scaled(model=model, nombre="tcn_cov", val_size=VAL_SIZE, salida=salida, series=series_ed_scaled, train_series=train_ed_scaled, val_series=val_ed, val_series_scaled=val_ed_scaled, covariates_series=covariates_series, train_covariates=train_covariates, val_covariates=val_covariates, scaler=scaler_ed)
        print(m_mae)
        print(m_mase)
        print(m_dtw)
        print(t_entrenamiento)
